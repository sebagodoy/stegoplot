...
import stegoplot.thermo_base
import stegoplot.promps
import numpy
from stegoplot.item_types import SingleItem
from stegoplot.parameters import kb, eV2J


class GasItem(SingleItem):
    def __init__(self, **kwargs):
        stegoplot.item_types.SingleItem.__init__(self, **{**kwargs, 'type': 'Gas'})
    ...
    def q_tras(self, T, P, Report=False, **_unused) -> 'float, Adim':
        try:
            out = stegoplot.thermo_base.qTras(self.mass, T, P)
            stegoplot.promps.report(Report, 'q_tras', out, self)  # report?
            return out
        except TypeError:
            stegoplot.promps.errorreport('Something is missing to compute q_tras for ' + self.Name)

    def q_rot(self, T, Report=False, **_unused) -> 'float, Adim':
        try:
            out = stegoplot.thermo_base.qRot(self.Geometry, self.RotTemp, T, self.RotSym)
            stegoplot.promps.report(Report, 'q_rot', out, self)  # report?
            return out
        except TypeError:
    ...
    def q_vib(self, T, Report=False, **_unused) -> 'float, Adim':
        # Check input
        try:
            out = stegoplot.thermo_base.qVib(self.FreqR, T)
            stegoplot.promps.report(Report, 'q_vib', out, self)  # report?
            return out
        except TypeError:
            NameError('Need a list of frea freqs[cm-1] and T[K] to compute qTras')

    def q_el(self, Report=False, **_unused):
        out = stegoplot.thermo_base.qEl()
        stegoplot.promps.report(Report, 'q_el', out, self)  # report?
        return out
    ...
    def Etras(self, T=None, Report=False, **_unused) -> 'float, eV':
        try:
            out = stegoplot.thermo_base.Etras(T)  # /[eV]
            stegoplot.promps.report(Report, 'E. tras.', out, self)  # report?
            return out
        except TypeError:
    ...
    def Erot(self, T=None, Report=False, **_unused) -> 'float, eV':
        try:
            out = stegoplot.thermo_base.Erot(self.Geometry, T=T)
            stegoplot.promps.report(Report, 'E. rot.', out, self)  # report?
            return out
        except TypeError:
    ...
    def ZPVE(self, Report=False, **_unused):
        # Check input
        try:
            out = stegoplot.thermo_base.ZPVE(self.FreqR)  # /[eV]
            stegoplot.promps.report(Report, 'ZPVE', out, self)  # report?
            return out
        except TypeError:
    ...
    def Evib(self, T=None, Report=False, **_unused):
        # Check input
        try:
            out = stegoplot.thermo_base.Evib(self.FreqR, T)
            stegoplot.promps.report(Report, 'E. vib', out, self)  # report?
            return out
        except TypeError:
    ...
    def Eel(self, Report=False, **_unused):
        ...
        stegoplot.promps.report(Report, 'E. el.', 0., self)  # report?
        return 0.
    ...
    def Stras(self, T=None, P=None, Report=False):
        ...
        try:
            ...
            stegoplot.promps.report(Report, 'S. tras.', out, self)  # report?
            return out
        except TypeError:
    ...
    def Srot(self, T, Report=False, **_unused):
        # Check input
        try:
            out = stegoplot.thermo_base.Srot(self.Geometry, self.RotTemp, T, self.RotSym)
            stegoplot.promps.report(Report, 'S. rot.', out, self)  # report?
            return out
        except TypeError:
    ...
    def Svib(self, T, Report=False, **_unused):
        # Check input
        try:
            out = stegoplot.thermo_base.Svib(self.FreqR, T)  # /[eV/K]
            stegoplot.promps.report(Report, 'S. vib.', out, self)  # report?
            return out
        except TypeError:
    ...
    def Sel(self, Report=False, **_unused):
        # Bottom of well reference
        # Negligible population of higher levels is assumed
        stegoplot.promps.report(Report, 'S. el.', 0., self)  # report?
        return 0
    ...
    def Internal(self, T=None, Report=False, **_unused):
        ...
        out = self.E0 \
              ...
              + self.ZPVE() \
              + self.Evib(T=T)
        stegoplot.promps.report(Report, 'Internal E.', out, self)  # report?
        return out

    def Enthalpy(self, T=None, Report=False, **_unused):
        # Includes pressure effect: H = U + kbT
        out = self.Internal(T=T) + kb * T / eV2J  # [eV]
        stegoplot.promps.report(Report, 'Enthalpy', out, self)  # report?
        return out
    ...
    def Entropy(self, T=None, P=None, Report=False):
        ...
        out = self.Stras(T, P) \
              + self.Srot(T) \
              + self.Svib(T)  # eV/K
        stegoplot.promps.report(Report, 'Entropy', out, self)  # report?
        return out
